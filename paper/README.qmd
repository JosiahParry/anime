...
The algorithm leverages R*-tree spatial indexing with angle-based and distance-based filtering criteria, combined with overlap computation methods that adapt to line segment orientation.
The algorithm calculates relationship metrics: shared length ($SL_{ij}$), source-weighted proportions ($sw_{ij}$), and target-weighted proportions ($tw_{ij}$), enabling both extensive and intensive interpolation methods for statistically sound attribute transfer.
...
Such datasets are used in many fields, including transport planning, which increasingly requires the integration of multiple representations of linear transport networks from diverse sources, such as those from different government agencies or commercial providers.
...
Several persistent challenges complicate the matching process: matching generalized geometry to more detailed representations [@kim_conflation_2017; @mustiere_matching_2008], handling cases where a single real-world feature may be represented differently across datasets (e.g., a multi-lane highway as a single line in one dataset and multiple parallel lines in another [@zhang_methods_nodate]), and managing complex one-to-many or many-to-many correspondence scenarios.
...
1.  **Develop a quantitative correspondence framework** that measures degrees of geometric overlap rather than binary classifications, enabling precise representation of partial correspondences between network features.
...
**Target Tree**:
$$Tree_B = \{(L_{jl}, j, m_{jl}, \text{AABB}_{jl}^{\text{expanded}}) : L_{jl} \in B\} \tag{13}$$
