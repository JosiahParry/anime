...
Feature-based approaches [@samal_feature_2004; @mustiere_matching_2008] improved upon these methods by incorporating both geometric and topological properties, introducing the concept of partial (1:M or M:N) matches through bottom-up (local geometric) and top-down (global topologic) procedures.
...
Graph-based and multi-stage approaches have also emerged: @nguyen_apsg_2022 introduced an area partitioning and subgraph growing (APSG) method that uses Voronoi diagrams to break regions into localities, achieving high precision and recall (F1-scores > 0.9) by explicitly handling missing features and representation differences.
