...
Although such data may be incomplete or geometrically imprecise, they can supplement missing information or capture recent infrastructure changes.
Recent assessments confirm that even mature crowd-sourced datasets like OpenStreetMap exhibit significant spatial variation in completeness and positional accuracy, with rural areas showing substantially lower data quality than urban regions [@li_osm_2025; @barrington_leigh_osm_2017].
...
Feature-based approaches [@samal_feature_2004; @mustiere_matching_2008] advanced the field by incorporating both geometric and topological properties, introducing partial (1:m or m:n) matches through combined bottom-up (local geometric) and top-down (global topologic) procedures.
...
Recent advances have demonstrated the continued importance of R-tree indexing for network operations, with novel implementations achieving substantial performance improvements for spatial joins and nearest-neighbor searches on road network data [@kim_sgirtree_2024].
...
Subsequent automated comparison frameworks have enabled systematic assessment of OSM against authoritative road datasets, revealing both the strengths and limitations of crowd-sourced data [@brovelli_osm_2016].
At a global scale, OSM has achieved over 80% road length completeness, though coverage remains uneven between developed and developing regions [@barrington_leigh_osm_2017].
...
Recent calls for open geographic information science practices highlight how sharing data, code, and workflows not only improves reproducibility but also accelerates innovation through community-driven enhancements [@holler_open_2025].
